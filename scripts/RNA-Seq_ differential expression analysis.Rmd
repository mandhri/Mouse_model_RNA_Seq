...
* Female DE: 351 genes at FDR≤0.05, Male has fewer (29)
...
* The histogram of LFC shows that the values of the res_F_raw and res_M_raw is centred around zero. 
...
# Feamles
sig<- res_F_raw$padj <= 0.05
table(sig)
big<- abs(res_F_raw$log2FoldChange) >= 0.5
table(big)

# Males

sig<- res_M_raw$padj <= 0.05
table(sig)
big<- abs(res_M_raw$log2FoldChange) >= 0.5
table(big)


plot(res_F_raw$log2FoldChange, res_M_raw$log2FoldChange, pch=16, cex=0.4)
abline(0,1,lty=2) 
...
summary(res_F_raw$lfcSE); summary(res_M_raw$lfcSE)
