...
```{r,load1}
gs_hallmark_mm <- msigdbr(
  species  = "Mus musculus",
  category = "H"
) %>%
  split(.$gene_symbol)

length(gs_hallmark_mm)

```

# Extract the zscors and the gene names for each of the contrasts

```{r}

tab_F <- as.data.frame(res_F_raw) %>%
  rownames_to_column("ensgene") %>%
  dplyr::select(ensgene, F_AngII_vs_Saline = stat)

tab_M <- as.data.frame(res_M_raw) %>%
  rownames_to_column("ensgene") %>%
  dplyr::select(ensgene, M_AngII_vs_Saline = stat)

tab_Int <- as.data.frame(res_int_raw) %>%
  rownames_to_column("ensgene") %>%
  dplyr::select(ensgene, Interaction_M_minus_F = stat)

tab_Bas <- as.data.frame(res_bsl_raw) %>%
  rownames_to_column("ensgene") %>%
  dplyr::select(ensgene, Baseline_M_vs_F = stat)
...
rna_sym <- rna_stats %>%
  ...
  dplyr::left_join(geneTable, by = "ensgene") %>%
  dplyr::filter(!is.na(symbol)) %>%
  dplyr::distinct(symbol, .keep_all = TRUE)

vec_F   <- rna_sym$F_AngII_vs_Saline
names(vec_F) <- rna_sym$symbol

vec_M   <- rna_sym$M_AngII_vs_Saline
names(vec_M) <- rna_sym$symbol

vec_Int <- rna_sym$Interaction_M_minus_F
names(vec_Int) <- rna_sym$symbol

vec_Bas <- rna_sym$Baseline_M_vs_F
names(vec_Bas) <- rna_sym$symbol

scores_list <- list(
  F_AngII_vs_Saline     = vec_F,
  M_AngII_vs_Saline     = vec_M,
  Interaction_M_minus_F = vec_Int,
  Baseline_M_vs_F       = vec_Bas
)


mk_ps <- function(mat, col) {
  data.frame(
    gene = rownames(mat),
    stat = as.numeric(mat[, col]),
    stringsAsFactors = FALSE
  )
}

scores_list <- list(
  F_AngII_vs_Saline     = mk_ps(rna_sym, "F_AngII_vs_Saline"),
  M_AngII_vs_Saline     = mk_ps(rna_sym, "M_AngII_vs_Saline"),
  Interaction_M_minus_F = mk_ps(rna_sym, "Interaction_M_minus_F"),
  Baseline_M_vs_F       = mk_ps(rna_sym, "Baseline_M_vs_F")
)


head(scores_list$F_AngII_vs_Saline)
