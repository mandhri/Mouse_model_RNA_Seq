...
```{r heatmap_raw, fig.width=13, fig.height=11, message=FALSE, warning=FALSE, echo=TRUE}

top_heatmap<- function(res_raw, vsd, label) {
  keep<- order(res_raw$padj, na.last = NA)[1:min(sum(!is.na(res_raw$padj)))]
  genes<- rownames(res_raw)[keep]
...
save_full(res_F_raw, ann, "/mnt/vol1/Mouse_model_RNA_Seq/Contrast_heatmap_Results/DESeq2_F_AngII_vs_Saline_full.csv")
...
Heatmaps were made to show the genes with the strongest evidence of differential expression in each contrast. Shrunken log2 fold changes were used for the heatmap in this section. 
For every contrast, all the genes were taken, variance-stabilised counts were scaled per gene, and samples were labelled by Sex and Treatment.

```{r heatmap_shr, fig.width=20, fig.height=20, message=FALSE, warning=FALSE, echo=TRUE}

top_heatmap<- function(res_shr, vsd, label) {
  keep<- order(res_shr$padj, na.last = NA)[1:min(sum(!is.na(res_shr$padj)))]
  genes<- rownames(res_shr)[keep]
  mat<- assay(vsd)[genes, , drop=FALSE]
  mat<- scale(t(scale(t(mat))))  
  ann<- as.data.frame(colData(vsd))[, c("Sex","Treatment"), drop=FALSE]
