...
ann_col<- as.data.frame(colData(dds))[, c("Sex","Treatment"), drop = FALSE]
...
# Heatmaps of all the genes per contrast 


## Heatmaps (Raw logFC)

### Heatmaps (all the genes)

Heatmaps were made to show the genes with the strongest evidence of differential expression in each contrast. Raw log2 fold changes were used for the heatmap in this section.  
For every contrast, all the genes were taken, variance-stabilised counts were scaled per gene, and samples were labelled by Sex and Treatment.

```{r heatmap_raw, echo=TRUE, fig.height=11, fig.width=13, message=FALSE, warning=FALSE}

top_heatmap <- function(res_tbl, vsd, label,
                        zcap = 3, show_rownames = FALSE,
                        cluster_rows = TRUE, cluster_cols = TRUE,
                        fontsize_row = 6) {
  keep  <- order(res_tbl$padj, na.last = NA)
  genes <- intersect(rownames(res_tbl)[keep], rownames(vsd))
  mat <- assay(vsd)[genes, , drop = FALSE]
  mat <- scale(t(scale(t(mat))))
  mat[mat >  zcap] <-  zcap
  mat[mat < -zcap] <- -zcap
  ann_col <- as.data.frame(colData(vsd))[, c("Sex","Treatment"), drop = FALSE]
  pheatmap::pheatmap(
    mat,
    annotation_col = ann_col,
...
top_heatmap(res_F_raw, vsd, "F Ang-II vs Saline")
top_heatmap(res_M_raw, vsd, "M Ang-II vs Saline")
top_heatmap(res_int_raw, vsd, "Interaction (M & F)")
top_heatmap(res_bsl_raw, vsd, "Baseline (M vs F, Saline)")


save_full <- function(res, ann, file){
  as.data.frame(res)  %>%
    tibble::rownames_to_column("ensgene")  %>%
    dplyr::mutate(ensgene = sub("\\.\\d+$","", ensgene))  %>%
    dplyr::left_join(ann, by = "ensgene")  %>%
    dplyr::select(symbol, ensgene, baseMean, log2FoldChange, lfcSE, stat, pvalue, padj)  %>%
    dplyr::arrange(padj)  %>%
    readr::write_csv(file)
}


save_full(res_F_raw, ann, "/mnt/vol1/Mouse_model_RNA_Seq/Contrast_heatmap_Results/DESeq2_F_AngII_vs_Saline_raw.csv")
save_full(res_M_raw, ann, "/mnt/vol1/Mouse_model_RNA_Seq/Contrast_heatmap_Results/DESeq2_M_vs_Saline_raw.csv")
save_full(res_int_raw, ann, "/mnt/vol1/Mouse_model_RNA_Seq/Contrast_heatmap_Results/DESeq2_Interaction_raw.csv")
save_full(res_bsl_raw, ann, "/mnt/vol1/Mouse_model_RNA_Seq/Contrast_heatmap_Results/DESeq2_Baseline_raw.csv")

```


### Heatmaps (significant genes only)
...
```{r heat_sig, fig.width=11, fig.height=9, message=FALSE, warning=FALSE, echo=TRUE}
...
Heatmaps use shrunken log2 fold changes for clearer, more stable effect sizes. For each contrast, all genes were included; variance-stabilised counts were z-scored per gene and samples annotated by Sex and Treatment.

```{r heatmap_shr, fig.width=13, fig.height=11, message=FALSE, warning=FALSE, echo=TRUE}
...
save_full <- function(res, ann, file){
  as.data.frame(res)  %>%
    ...
    dplyr::mutate(ensgene = sub("\\.\\d+$","", ensgene))  %>%
    dplyr::left_join(ann, by = "ensgene")  %>%
    dplyr::select(symbol, ensgene, baseMean, log2FoldChange, lfcSE, pvalue, padj)  %>%
    dplyr::arrange(padj)  %>%
    readr::write_csv(file)
}


save_full(res_F_shr,ann, "/mnt/vol1/Mouse_model_RNA_Seq/Contrast_heatmap_Results/DESeq2_F_vs_Saline_shrunk.csv")
save_full(res_M_shr,ann, "/mnt/vol1/Mouse_model_RNA_Seq/Contrast_heatmap_Results/DESeq2_M_vs_Saline_shrunk.csv")
save_full(res_int_shr,ann, "/mnt/vol1/Mouse_model_RNA_Seq/Contrast_heatmap_Results/DESeq2_Interaction_shrunk.csv")
save_full(res_bsl_shr,ann, "/mnt/vol1/Mouse_model_RNA_Seq/Contrast_heatmap_Results/DESeq2_Baseline_shrunk.csv")
...
```{r het_sig_shr, fig.width=11, fig.height=9, message=FALSE, warning=FALSE, echo=TRUE}
...
readr::write_csv(dplyr::arrange(gsea_F,  padj, pval) %>% dplyr::select(-leadingEdge),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/fgsea_hallmark_full_F_vs_Saline.csv")
readr::write_csv(dplyr::arrange(gsea_M,  padj, pval) %>% dplyr::select(-leadingEdge),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/fgsea_hallmark_full_M_vs_Saline.csv")
readr::write_csv(dplyr::arrange(gsea_Int,padj, pval) %>% dplyr::select(-leadingEdge),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/fgsea_hallmark_full_Interaction.csv")
readr::write_csv(dplyr::arrange(gsea_Bas,padj, pval) %>% dplyr::select(-leadingEdge),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/fgsea_hallmark_full_Baseline.csv")
...
readr::write_csv(dplyr::arrange(gsea_F,  padj, pval) %>% dplyr::select(-leadingEdge),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/fgsea_GO_BP_full_F_vs_Saline.csv")
readr::write_csv(dplyr::arrange(gsea_M,  padj, pval) %>% dplyr::select(-leadingEdge),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/fgsea_GO_BP_full_M_vs_Saline.csv")
readr::write_csv(dplyr::arrange(gsea_Int,padj, pval) %>% dplyr::select(-leadingEdge),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/fgsea_GO_BP_full_Interaction.csv")
readr::write_csv(dplyr::arrange(gsea_Bas,padj, pval) %>% dplyr::select(-leadingEdge),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/fgsea_GO_BP_full_Baseline.csv")
...
readr::write_csv(dplyr::arrange(hall_all, p.adjustANOVA, pANOVA),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/mitch_hallmark_full_all_contrasts.csv")
...
readr::write_csv(dplyr::arrange(gobp_all,  p.adjustANOVA, pANOVA),
                 "/mnt/vol1/Mouse_model_RNA_Seq/pathway_Results/mitch_gobp_full_all_contrasts.csv")
